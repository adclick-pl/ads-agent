/**
 * Search Console OAuth — two explicit steps.
 *
...
if (step === 'url') {
  ...
  console.log('\n🔐 Autoryzacja Search Console (zakres: webmasters.readonly — tylko odczyt)\n');
  ...
  console.log('1. Otwórz ten link i zaloguj się kontem, które ma dostęp do property w GSC:\n');
  console.log(authUrl());
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    res.end('<h2>✓ Gotowe</h2><p>Token Search Console zapisany. Możesz zamknąć to okno.</p>');
    server.close();
    console.log(`\n✓ Token zapisany: ${path}`);
    console.log(
  ...
  if (e.code === 'EADDRINUSE') {
    console.error(
      `\n❌ Port ${PORT} jest zajęty — najczęściej przez inny nasłuch autoryzacji\n` +
        `(konektor Google Ads i GA4 używają tego samego portu).\n` +
